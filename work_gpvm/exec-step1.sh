...
for (( RUN_I = 0 ; RUN_I < $RUN_N ; RUN_I++ )) ; do
	...
	if [ $DO_OVERWRITE = yes ] ; then
	    echo "  Clean up."
	    rm -rf $DIR_WORK_RUN
	elif [ ! -e $DIR_WORK_RUN/out/finished.txt ] ; then
	    echo "  Clean up since incomplete."
	    rm -rf $DIR_WORK_RUN
	else
	    echo "  Skip."
	    continue
	...
	test $USE_RAW = yes && CMD+=" -f $FN_RAW"
	test $USE_REC = yes && CMD+=" -f $FN_REC"
	...
	test $USE_RAW = yes && ln -nfs $FN_RAW $DIR_WORK_RUN/in
	test $USE_REC = yes && ln -nfs $FN_REC $DIR_WORK_RUN/in
